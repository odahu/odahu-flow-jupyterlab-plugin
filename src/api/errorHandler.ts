...
import {
  ODAHUFLOW_OAUTH_TOKEN_COOKIE_NAME,
...
export function odahuflowHttpExceptionHandler() {
  return (
    target: any,
    propertyKey: string,
    ...
    descriptor.value = async function() {
      try {
        ...
        // If we get 403 from odahuflow plugin then the token is obsolete.
        if (
          err instanceof ResponseError &&
          err.response.status === HTTP_STATUS_CODE_FORBIDDEN
        ) {
          // Cleanup odahuflow token from local storage store
          localStorage.removeItem(PLUGIN_CREDENTIALS_STORE_TOKEN);
          // Cleanup odahuflow token from cookie
          document.cookie = `${ODAHUFLOW_OAUTH_TOKEN_COOKIE_NAME}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
        }
