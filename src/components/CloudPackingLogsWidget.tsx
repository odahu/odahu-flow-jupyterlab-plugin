...
const WIDGET_ID = 'odahuflow:cloud-packaging:logs:';
const WIDGET_TITLE = 'Logs of :packagingName:';
const WIDGET_UPDATE_TIME = 5000;

export class CloudPackagingLogsWidget extends BaseOdahuflowWidget {
  private _data: ICloudLogsResponse;
  private _api: ICloudApi;
  private _timerId?: number;
  private _state: IApiCloudState;
  private _packagingName: string;
  ...
  constructor(
    ...
    this._api = api;
    this._state = state;
    this._packagingName = packagingName;
    ...
    this.id = WIDGET_ID + packagingName;
    this.title.label = WIDGET_TITLE.replace(':packagingName:', packagingName);
  ...
  refreshData() {
    ...
    this._api
      ...
      .catch(err => {
        this._finishUpdating();
        console.error('Can not update packaging logs', err);
      });
  }
