...
class LoginDialog extends Widget {
   ...
   * @param tokenRequired - If it equals true then the widget creates input field for a Odahuflow token.
   */
...
export function showLoginDialog(
  ...
  return showDialog({
    title: 'Authorization on a Odahuflow cluster',
...
export function showLogoutDialog(clusterName) {
  return showDialog({
    title: 'Logging out on a Odahuflow cluster',
    body: `Do you want to log out on odahuflow cluster ${clusterName}?`,
    buttons: [
      Dialog.cancelButton(),
